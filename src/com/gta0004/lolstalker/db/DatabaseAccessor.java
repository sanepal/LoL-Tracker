...
public class DatabaseAccessor {
	...
	public List<SummonerDto> getAllSummoners() {
		...
		open();
        /*SELECT app_items.id, app_items.title, app_items.time FROM app_items 
        JOIN app_item_year ON app_item_year.item=app_items.id
        WHERE app_item_year.year = ?*/
        ...
        Cursor cursor = query.query(
                            database, 
                            new String[] {
                                    DatabaseHelper.COLUMN_SUMM_ID, 
                                    DatabaseHelper.COLUMN_SUMM_NAME, 
                                    DatabaseHelper.COLUMN_SUMM_IC,
                                    DatabaseHelper.COLUMN_SUMM_REV,
                                    DatabaseHelper.COLUMN_SUMM_LEVEL,
                                    DatabaseHelper.COLUMN_SUMM_LAST,
                                    }, 
                            null,
                            null, 
                            null, 
        ...
        while (!cursor.isAfterLast()) {
        	SummonerDto summoner = new SummonerDto(cursor.getLong(0),cursor.getString(1),cursor.getInt(2), cursor.getLong(3), cursor.getInt(4));
